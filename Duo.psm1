...
function duoCreateUser()
{
    ...
    [string[]]$param = "email","username","alias1","alias2","alias3","realname","status","notes"
    ...
    foreach ($p in $param)
    {
        if (Get-Variable -Name $p -ErrorAction SilentlyContinue) 
        {
            if ((Get-Variable -Name $p -ValueOnly) -ne "" -or $p -notlike "alias")
            {
                $parameters.Add($p,(Get-Variable -Name $p -ValueOnly))
            }
            elseif ((Get-Variable -Name $p -ValueOnly) -ne "" -and $p -like "alias")
            {
                if ($counter -eq 1)
                {
                    $aliases = $p + "=" + (Get-Variable -Name $p -ValueOnly)
                    $counter++
                } 
                else
                {
                    $aliases = $aliases + "&" + $p + "=" + (Get-Variable -Name $p -ValueOnly)
                }
            }
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwvbcaFYTCk/0KELDbLA3W6df
# 0BygghiwMIIE/jCCA+agAwIBAgIQDUJK4L46iP9gQCHOFADw3TANBgkqhkiG9w0B
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBSp/o7rIlBY
# +uXBTUnYMCdVn7ls3TANBgkqhkiG9w0BAQEFAASCAQBDfm0OZz+pkAQjW7TWSw2r
# iAxk5oFzpStFEAHeT5+tvKrOMHccnn87LC1M/5iy98HwKrzem+M7UL/YunNkwtgs
# fZDgONB45IW5C9xHm9fmdKQud9i3dVjDrqImRdXxB6olHUREArI225Ng+YpgAs1O
# sASEDzfNzWSjtBCskgcqSXllPTYf2PgWdTsIiWnpg4b7Ewv00sAcbLji5QYTBgCZ
# m+vLVX1aAhKu9gYbGZoYDFMwVdQo0oIp13NAWfyua3JbiclfMIhOo3pJ/7beEit1
# yhC7pRjwtZU/Y3iFUd/rXBrtfMJdrc33Q/5DfgGBpXD4Zqhow+MMtkh85N2XeSuB
...
# 9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMTExMjYxNTQwMjda
# MC8GCSqGSIb3DQEJBDEiBCCz1RBQBe/zCiOSPwSrU7e/qJ+T72z23WCEsnbnwajD
# 0DANBgkqhkiG9w0BAQEFAASCAQBRnqYF1DrTI7y47ZzRQpmH+ZyVekk5zAzwzs7C
# fBvOeieJF1SRjNhO+vszlR3efe1WfVt4LJh0thXSewtqyUnWjkZ834dsLhRVZ9SU
# bwSkuNC9cTdMX5UINiRrOlGCzWPkvUlDu46Ft4ibIUHiZ4dW4ReWj+AQk0JUvyII
# UwqPLxJjkHsY4cmhKQ12DjiWQrR+MlitM7cK796GKo14v2HDYUemmwRxtxnrjDm/
# IOvs/yyzFWcvO4VeD4ke55168Nlpeji/et0aSrKh1oI0Feuj5Mj3XyEwAp0g0/ZZ
# ceootgxfR/J59xfKw4Kauc6YTw/wzSoRAf9VRUlL1sJbpDcd
